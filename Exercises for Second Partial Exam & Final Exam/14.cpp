...
public:
    ...
    SMS(char tel[16], float basePrice)
    {
        strcpy(this->tel, tel);
    ...
    friend ostream &operator<<(ostream &res, SMS &sms)
    {
        res << "Tel: " << sms.tel << " - cena: " << sms.SMS_cena() << "den." << endl;
        return res;
    }
};
...
public:
    ...
    RegularSMS(char tel[16], float basePrice, char *message, bool roam)
        : SMS(tel, basePrice)
    {
    ...
    float SMS_cena()
    {
        float x = roam ? mult : ddv;
        return basePrice * (1.0 + x) * ceil(strlen(message) / 160.0);
    }
};
...
public:
    ...
    SpecialSMS(char tel[16], float basePrice, bool humane)
        : SMS(tel, basePrice)
    {
        this->humane = humane;
    }
...
{
    int reg = 0, special = 0;
    float regSum = 0, specialSum = 0;
    for (int i = 0; i < n; i++)
    {
        if (dynamic_cast<RegularSMS *>(poraka[i]))
        {
            reg++;
            regSum += poraka[i]->SMS_cena();
        }
        else
        {
            special++;
    ...
    cout << "Vkupno ima " << reg << " regularni SMS poraki i nivnata cena e: " << regSum << endl;
    cout << "Vkupno ima " << special << " specijalni SMS poraki i nivnata cena e: " << specialSum << endl;
}
