...
public:
    ...
    Pizza(char name[20], char sostojki[100], double basePrice)
    {
    ...
    virtual string getSostojki()
    {
        return string(sostojki);
    }
    ...
    friend ostream &operator<<(ostream &res, const Pizza &pizz);
};
...
public:
    ...
    FlatPizza(char name[20], char sostojki[100], double basePrice)
        : Pizza(name, sostojki, basePrice)
    {
        this->size = small;
    }

    FlatPizza(char name[20], char sostojki[100], double basePrice, Size size)
        : Pizza(name, sostojki, basePrice)
    {
        this->size = size;
    }
...
public:
    FoldedPizza() : Pizza()
    {
        this->whiteFlower = true;
    }

    FoldedPizza(char name[20], char sostojki[100], double basePrice)
        : Pizza(name, sostojki, basePrice)
    {
        this->whiteFlower = true;
    }

    FoldedPizza(FoldedPizza &other)
        : Pizza(other)
    {
        this->whiteFlower = other.whiteFlower;
    }

    ~FoldedPizza() {}

    void setWhiteFlour(bool newValue)
    {
        this->whiteFlower = newValue;
    }
...
ostream &operator<<(ostream &res, Pizza &pizza)
{
    res << pizza.getName() << ": " << pizza.getSostojki();
    if (dynamic_cast<FlatPizza *>(&pizza))
    {
        res << ", " << dynamic_cast<FlatPizza *>(&pizza)->getSize();
    }
    else if (dynamic_cast<FoldedPizza *>(&pizza))
    {
        res << ", " << (dynamic_cast<FoldedPizza *>(&pizza)->getWhiteFlower() ? "wf" : "nwf");
    }

    res << " - " << pizza.price() << endl;
...
{
    int res = 0;
    float curr = pizzas[res]->price();

    for (int i = 0; i < numPizzas; i++)
    {
        if (curr < pizzas[i]->price())
        {
            curr = pizzas[i]->price();
            res = i;
        }
