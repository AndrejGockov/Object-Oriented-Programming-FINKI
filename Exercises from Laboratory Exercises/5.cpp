...
class Chocolate{
    ...
    public:
        ...
        friend ostream &operator<<(ostream &res, const Chocolate &choc){
            res << choc.name << ": $" << choc.price;
            return res;
        }
};
...
class ChocolateFactory{
    ...
    public:
        ...
        ChocolateFactory(Chocolate *products, int *weeklyProduction, int numProducts){
            ...
            this->weeklyProduction = new int[this->numProducts];
            this->products = new Chocolate[this->numProducts];
        ...
        bool operator<(ChocolateFactory &other){
        ...
        ChocolateFactory &operator+(ChocolateFactory &other){
            int newNumProducts = this->numProducts + other.numProducts;
            int *tmpProduction = new int[this->numProducts];
            Chocolate *tmpProducts = new Chocolate[this->numProducts];
            for(int i = 0; i < this->numProducts; i++){
                tmpProduction[i] = this->weeklyProduction[i];
                tmpProducts[i] = this->products[i];
            }
            delete[]this->products;
            delete[]this->weeklyProduction;
            this->products = new Chocolate[newNumProducts];
            this->weeklyProduction = new int[newNumProducts];
            
            for(int i = 0, j = 0; i < newNumProducts; i++){
                if(i < this->numProducts){
                    this->weeklyProduction[i] = tmpProduction[i];
                    this->products[i] = tmpProducts[i];
                }else{
                    this->weeklyProduction[i] = other.weeklyProduction[j];
                    this->products[i] = other.products[j];
                    j++;
                }
            }
            
            this->numProducts = newNumProducts;
            delete[]tmpProduction;
            delete[]tmpProducts;
            
            return *this;
        }
        
        friend ostream &operator<<(ostream &res, ChocolateFactory &chocFac){
            for(int i = 0; i < chocFac.numProducts; i++){
                res << chocFac.products[i] << " x " << chocFac.weeklyProduction[i] << endl;
            }
            res << "$" << chocFac.weeklyIncome() << "/wk" << endl;
            return res;
        }
};
